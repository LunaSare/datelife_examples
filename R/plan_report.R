...
for(i in seq(taxa)){
    ...
    plan_report <- drake_plan(
      ...
      tax_median_phylo = tax_median_phyloall[[i]]$phylo_median,
      ...
      plot3 = make_plot_global(tree = tax_otolall[[i]], title = NULL, taxon = taxa[i], tax_summall[[i]], omi3 = 0, filename = "otol"),
    	...
    	lttplot_sdm2phy = make_lttplot_sdm(taxa[i], tax_phyloall, tax_datedotol, tax_phylomed = tax_median_phylo,
    	...
    	lttplot_summ_med = make_lttplot_summ(taxa[i], tax_phyloall, tax_datedotol, 
    	                                   tax_phylosummary = tax_median_phyloall[[i]],
