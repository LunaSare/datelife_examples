lttplot_summaries2 <- function(taxon, trees, summs, summs_title, summs_col = "#0000FF",
  xlim0, tax_phyloall, tax_summary, study_number, max_tips, max_ages, col_phyloall_sample,
  add_legend = FALSE){
    ...
    lab <- paste(gsub("_tree", " ", names(summs)), ifelse("median" %in% tolower(summs_title),
      tolower(summs_title), summs_title))
    for (i in seq(summs)){
      foo(phy = summs[[i]], color_here = summs_col, labels_here = lab[i], length_arrowhead, max_tips)
    }
    ...
    mtext(text = paste(summs_title, "summary chronograms"), side = 3, cex = 1, font = 2, line = -1.5)
    if(add_legend){
        leg <- paste(taxon, c("source chronograms", "summary chronograms"))
        legend(x = -xlim0, y = max_tips*1.35, legend = leg, cex = 0.75, pch = 19,
            bty = "n", xpd = NA, col = c("#77889980", paste0(summs_col, 80)), inset = -1)
    }
}

make_lttplot_summchrono2 <- function(taxon, tax_phyloall, tax_summary, sdm_summs3, median_summs3,
        filename = "LTTplot_summary_chronograms2",
    ...
    par(omi = c(0.75,0,0.35,0))
    par(mfrow = c(2,1))
    par(mai = c(0.25, 0.82, 0.25, 0.2))
    lttplot_summaries2(taxon, trees, median_summs3, summs_title = "Median",
        summs_col = "#0000FF", xlim0, tax_phyloall, tax_summary, study_number, max_tips,
        max_ages, col_phyloall_sample, add_legend = TRUE)
    lttplot_summaries2(taxon, trees, sdm_summs3, summs_title = "SDM",
        summs_col = "#0000FF", xlim0, tax_phyloall, tax_summary, study_number, max_tips,
        max_ages, col_phyloall_sample, add_legend = FALSE)
