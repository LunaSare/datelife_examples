...
plan_data2 <- drake_plan(
  ...
  tax_phyloall_boldall2 = mapply(get_bold_trees, taxa, tax_phyloallall,
    rep(FALSE, length(tax_phyloallall))),
  crossval_pathd8_exp2 = mapply(use_othercals4, taxa, tax_phyloall_boldall2,
    tax_othercalall),
  crossval_pathd8_summ2 = mapply(use_othercals4, taxa, tax_phyloall_boldall2,
    tax_othercalall, rep(0, length(tax_phyloall_boldall2)))
)
make(plan_data2)
