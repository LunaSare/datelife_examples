...
figcapextra <- vector(mode = "list", length = 2)
for (i in negs_list){
  if(length(negs_list[[i]])>0){
    xx <- rownames(tax_summ_matrix[[i]])[ceiling(negs/nrow(tax_summ_matrix[[i]]))]
    cat(paste0("This taxon's", names(negs_list)[i], " matrix has some negative values in the following taxa: *", paste(xx, collapse = "*, *"), "*."))
    figcapextra[[i]] <- "As you can note, dashed lines and solid lines from trees coming out from both types of clustering algorithms implemented are mostly overlapping. This means that removing negative values does not change results from clustering algorithms much."
  } else {
    cat("This taxon's",  names(negs_list)[i], " matrix has NO negative values. ")
      figcapextra[[i]] <- ""
  }
}
...
figcap_lttplot_median <- paste(taxon, "lineage through time (LTT) plots from source chronograms and median summary matrix converted to phylo with different methods (NJ and UPGMA).", figcapextra[[1]], "Clustering algorithms used here are returning non-ultrametric trees that are too old. So we developped an alternative algorithm in `datelife` to go from a summary matrix to a fully ultrametric tree.")

lttplot_median <- paste0("\n![", figcap_lttplot_median, "](plots/", taxon, "_LTTplot_Median.pdf)\n")
cat(lttplot_median)
```
