...
# loadd(tax_crossval2)
figcap1 <- paste(taxon, "Species Dated Open Tree of Life Induced Subtree. This chronogram was obtained with `get_dated_otol_induced_subtree()` function.")
...
figcap_lttplot_sdm <- paste(taxon, "lineage through time (LTT) plots from source chronograms and SDM output converted to phylo with different methods (NJ and UPGMA).")
```
\raggedright
...
if(!inherits(tax_datedotol, "phylo")){
  ...
  plot1 <- paste0("\n![", figcap1, "](plots/", taxon, "_datedotol.pdf)\n")
}
cat(plot1)
```
...
## 1.a) Summarize query results: diagnosing SDM issues.
...
We also tried  each source chronogram independently, with the Dated OToL and with each other, as a form of cross validation in Table2. this is not working perfectly yet, but we are developping new ways to use all calibrations efficiently.
