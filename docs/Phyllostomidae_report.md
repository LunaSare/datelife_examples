...
In this data set we have 892 calibrations in total (that basically
...
Unfortunately, a tree with branch lengths could not be
    constructed for any of the source chronograms available for the Phyllostomidae, so this workflow will not be exemplified here. This can happen for several
...
Synthetic tree of Phyllostomidae and a tree was constructed, but all branch lengths are NA.
...
\caption{\label{tab:unnamed-chunk-6}Phyllostomidae source chronogram studies information.}\\
\toprule
...
![Lineage Through Time plots of Phyllostomidae median summary
...
the median summary matrix with all clustering algorithms (Appendix Fig. 5).](plots/Phyllostomidae_lttplot_cluster_median.pdf)
...
![Phyllostomidae lineage through time (LTT) plots from
    ...
    from respective summary matrices and then adjusting branch lengths with BLADJ.](plots/Phyllostomidae_LTTplot_summary_chronograms.pdf)
...
here with all algorithms (mainFig. 2).](plots/Phyllostomidae_lttplot_cluster_median.pdf)


Dated induced subtree could not be obtained for the Phyllostomidae.
...
![Phyllostomidae lineage through time (LTT) plots from source chronograms and SDM summary matrix converted to phylo with different methods (NJ and UPGMA).  Clustering algorithms used often are returning non-ultrametric trees or
  with maximum ages that are just off (too old or too young). So we developped an
  alternative algorithm in `datelife` to go from a summary matrix to a fully ultrametric tree.](plots/Phyllostomidae_LTTplot_sdm.pdf)
